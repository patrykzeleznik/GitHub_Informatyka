...
class Window(QWidget):
    def __init__(self):
        ...
        self.timer9 = QTimer()
        self.timer9.timeout.connect(lambda: self.podgrzewaj_z1())
        self.btn9 = QPushButton("Podgrzej", self)
        self.btn9.setGeometry(410, 260, 80, 30)
        self.btn9.setStyleSheet("background-color: #444; color: white; font-weight: bold;")
        self.btn9.clicked.connect(lambda: self.przelacz_symulacje(9))
        self.running9 = False
    ...
    def przelacz_symulacje(self, nr_procesu):
        if nr_procesu == 1:
            ...
            if self.running9:
                self.timer9.stop()
                self.btn9.setText("Wznow")
            else:
                self.timer9.start(20)
                self.btn9.setText("Zatrzymaj")
            self.running9 = not self.running9
            self.update()
    ...
    def pompowanie_cieczy(self, nr_procesu):
        ...
        if nr_procesu == 1:
            if not self.z1.czy_pelny():
                total = self.z1.aktualna_ilosc + 2
                if total > 0:
                    self.z1.temp = (self.z1.aktualna_ilosc * self.z1.temp) / total
                self.z1.dodaj_ciecz(2)
                pompowanie = True
            ...
            if not self.z2.czy_pelny():
                total = self.z2.aktualna_ilosc + 2
                if total > 0:
                    self.z2.temp = (self.z2.aktualna_ilosc * self.z2.temp) / total
                self.z2.dodaj_ciecz(2)
                pompowanie = True
            ...
            if not self.z3.czy_pelny():
                total = self.z3.aktualna_ilosc + 2
                if total > 0:
                    self.z3.temp = (self.z3.aktualna_ilosc * self.z3.temp) / total
            ...
            if not self.z4.czy_pelny():
                total = self.z4.aktualna_ilosc + 2
                if total > 0:
                    self.z4.temp = (self.z4.aktualna_ilosc * self.z4.temp) / total
                self.z4.dodaj_ciecz(2)
                pompowanie = True
            ...
            if not self.z5.czy_pelny():
                total = self.z5.aktualna_ilosc + 2
                if total > 0:
                    self.z5.temp = (self.z5.aktualna_ilosc * self.z5.temp) / total
    ...
    def odpompowanie_cieczy(self, nr_procesu):
        ...
        if nr_procesu == 1:
            if self.z5.aktualna_ilosc > 0.5:
                self.z5.usun_ciecz(0.2)
                odpompowanie = True
            ...
            if self.z4.aktualna_ilosc > 0.5:
                self.z4.usun_ciecz(0.2)
                odpompowanie = True
    ...
    def podgrzewaj_z1(self):
        if self.z1.aktualna_ilosc > 0:
            self.z1.temp = min(100, self.z1.temp + 4)
            self.update()
    ...
    def logika_przeplywu(self):
        ...
        plynie_1 = False
        plynie_2 = False
        if self.z1.aktualna_ilosc > 0.1:
            if not self.z2.czy_pelny():
                wsad = self.z1.usun_ciecz(out_speed)
                total_woda = self.z2.aktualna_ilosc + wsad
                if total_woda > 0:
                    nowa_temp = (self.z2.aktualna_ilosc * self.z2.temp + wsad * self.z1.temp) / total_woda
                    self.z2.temp = nowa_temp
                self.z2.dodaj_ciecz(wsad)
                plynie_1 = True
            if not self.z3.czy_pelny():
                wsad = self.z1.usun_ciecz(out_speed)
                total_woda = self.z3.aktualna_ilosc + wsad
                if total_woda > 0:
                    nowa_temp = (self.z3.aktualna_ilosc * self.z3.temp + wsad * self.z1.temp) / total_woda
                    self.z3.temp = nowa_temp
        ...
        if self.z2.aktualna_ilosc > 0.1 and not self.z5.czy_pelny():
            ...
            total_woda = self.z5.aktualna_ilosc + wsad
            if total_woda > 0:
                nowa_temp = (self.z5.aktualna_ilosc * self.z5.temp + wsad * self.z2.temp) / total_woda
                self.z5.temp = nowa_temp
        ...
        if self.z3.aktualna_ilosc > 0.1 and not self.z4.czy_pelny():
            ...
            total_woda = self.z4.aktualna_ilosc + wsad
            if total_woda > 0:
                nowa_temp = (self.z4.aktualna_ilosc * self.z4.temp + wsad * self.z3.temp) / total_woda
                self.z4.temp = nowa_temp
